...
export function monthsFromMembershipPaymentRow(row: {
  ...
  service: { monthlyRate: Prisma.Decimal | number };
}): number {
  ...
  if (monthlyRate > 0 && Number.isFinite(monthlyRate)) {
    return Math.max(1, Math.trunc(Math.round(gross / monthlyRate) || 1));
  }
  return 1;
}

export function sumManualLockInMonthsAfterAnchor(
  entries: Array<{ paidMonths: number; paidAt: Date }>,
  anchor: Date | null,
): number {
  return entries
    .filter((e) => (anchor == null ? true : e.paidAt.getTime() > anchor.getTime()))
...
export async function recomputeMemberLockInFields(tx: Prisma.TransactionClient, userId: string): Promise<void> {
  const user = await tx.user.findUnique({
    where: { id: userId },
    include: { lockInManualEntries: { select: { paidMonths: true, paidAt: true } } },
  });
  ...
  const template = Math.max(0, Math.trunc(Number(svc?.contractMonths) || 0));
  if (template <= 0) {
